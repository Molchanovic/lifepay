	...
	return gulp.src([source_folder + '/img/iconsprite/notedits/*.svg'])
		...
		.pipe(svgSprite({
			shape: {
				id: {
					whitespace: '_',
					//generator: function (name, file) { return "icon-" + name.replace('.svg', '') }
				}
			},
			mode: {
				symbol: {
					...
					render: {
						scss: {
							dest: '../../'+source_folder+'/scss/_sprite.scss',
...
function svgSpriteBuildEdit() {
	return gulp.src([source_folder + '/img/iconsprite/edits/*.svg'])
		// minify svg
		.pipe(svgmin({
			js2svg: {
				pretty: true
		...
		.pipe(svgSprite({
			...
			mode: {
				symbol: {
					sprite: "../img/icons/sprite-edits.svg",
					render: {
						scss: {
							dest: '../../'+source_folder+'/scss/_spriteEdit.scss',
							template: source_folder +"/scss/_sprite_template.scss"
						}
					}
				}
...
let build = gulp.series(clean, gulp.parallel(svgSpriteBuild, svgSpriteBuildEdit, otf2ttf, js, ht, txt, css, customCss, html, php, images, fonts, libs), fontsStyle);
...
exports.svgSpriteBuildEdit = svgSpriteBuildEdit;
exports.css = css;
